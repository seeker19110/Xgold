...
dùng đúng ý: _"Đây hiện là repo khung/template, chưa có dự án cụ thể để audit toàn diện. Audit
...
`PROJECT.md` → thiết kế) trước, rồi quay lại `/audit-full`."_ Gợi ý chạy `/consult` hoặc
...
(không phải file này — file này chỉ là _hướng dẫn chạy_, dùng chung cho mọi dự án).
