...
> Nói về model **chạy Claude Code để làm dự án theo khung này** — KHÔNG phải chọn model _bên trong_ sản phẩm bạn xây (cái đó xem kỹ năng `claude-api`).
...
| **Tầm trung → lớn** (10–50k+ LOC)            | **opusplan** (mặc định)       | _(không đổi gì)_                      |
...
Model (§2) là cần thứ nhất, effort (§4) là cần thứ hai; **cách vận hành phiên** là cần thứ ba — và tiết kiệm nhiều nhất, vì nó quyết định _bao nhiêu token phải nạp_, không chỉ _giá mỗi token_.
