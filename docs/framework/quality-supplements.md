...
gỡ rác, giảm trùng lặp & độ phức tạp, tỉa phụ thuộc, thu nhỏ bundle — một cách _có kỷ luật_, **không đổi hành vi**.

> **Phân biệt:** mục 2 = "trang chạy nhanh cho _người dùng_"; mục 9 = "mã dễ đọc, dễ sửa, ít rác cho _lập trình viên_".
...
- **Không đổi hành vi.** Refactor = đổi _cấu trúc_, giữ nguyên _kết quả_. Nếu phải đổi hành vi → đó là feature/fix, tách commit riêng.
- **Có lưới an toàn trước.** Phải có test phủ vùng sắp sửa _trước khi_ động vào. Chưa có test → viết test mô tả hành vi hiện tại (characterization test) trước, rồi mới refactor.
- **Bước nhỏ, xanh liên tục.** Mỗi bước nhỏ → chạy lại test → commit. Không refactor lớn trong một cú nhảy.
- **Đo trước–sau.** Có số liệu (số dòng, bundle size, số cảnh báo complexity, số dependency) trước & sau để chứng minh _thật sự_ gọn hơn, không chỉ "cảm giác".
...
_Trùng lặp & độ phức tạp:_

- [ ] Logic lặp ≥ 3 lần → tách dùng chung (DRY) — nhưng **đừng** gom thứ chỉ _trông_ giống nhau (tránh trừu tượng hóa sai, sau này khó tách).
...
> **Công cụ gợi ý (tùy chọn — chỉ tài liệu, _chưa_ ép trong CI):** `knip` (rác + export/dep thừa),
