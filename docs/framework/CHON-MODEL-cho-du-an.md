...
- **`.claude/agents/thuc-thi.md`** → subagent `model: sonnet`, **thực thi việc RÕ PHẠM VI** đã được phiên chính bóc tách (viết test theo spec, sinh boilerplate, cập nhật docs theo thay đổi đã biết, sửa cơ học nhiều file). Giá trị là **cô lập ngữ cảnh + chạy song song** để rút tải khỏi main Opus — **không** rẻ hơn pha-code opusplan (cùng Sonnet). Không giao việc kiến trúc/bảo mật/breaking change (để Opus, xem §9).
