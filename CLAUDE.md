...
- `.claude/commands/ke-hoach.md` — **lập kế hoạch & phạm vi (GĐ 1)**. **TRIGGER:** khi *chốt phạm vi MVP / viết đặc tả* (hoặc gõ `/ke-hoach`) → sinh `PROJECT.md` (MVP MoSCoW + tiêu chí chấp nhận đo được + DoD + sổ rủi ro) theo hồ sơ loại dự án; nối greenfield giữa `/tu-van` và `/khoi-tao`.
...
- `.claude/commands/kiem-thu.md` — **kiểm thử (GĐ 5)**. **TRIGGER:** khi *viết/rà test, lập chiến lược kiểm thử* (hoặc gõ `/kiem-thu`) → chọn cổng test theo hồ sơ (kim tự tháp, ca biên, E2E+a11y/contract, hiệu năng), mỗi bug → 1 test hồi quy (bám `BO-SUNG-chat-luong.md` Nhóm 2 mục 3/4/6).
- `.claude/commands/trien-khai.md` — **triển khai & ra mắt (GĐ 6–7)**. **TRIGGER:** khi *đưa lên production / chuẩn bị ra mắt* (hoặc gõ `/trien-khai`) → cổng CI/CD, migration + backup/rollback, checklist ra mắt (pháp lý/SEO/onboarding); thao tác khó hoàn tác → xin xác nhận (§9).
- `docs/framework/AP-DUNG-vao-du-an-co-san.md` + `.claude/commands/ap-dung.md` — áp khung lên **dự án đã có sẵn** (brownfield, tăng dần). **TRIGGER:** khi *áp repo/khung này vào dự án có sẵn* (hoặc gõ `/ap-dung`) → **chỉ tư vấn & nâng cấp** trên stack hiện có (tự dò stack từ repo, đo baseline, hạ nợ dần), **KHÔNG áp đặt** stack mặc định.
